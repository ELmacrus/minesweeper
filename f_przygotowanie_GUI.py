...
#globalne wartosci dla przyciskow z gui
global wierszGUI
global kolumnaGUI
...
def sprawdz_miny(plansza_gry, rozmiar_planszy):

	liczba_poprawek = 1
	while liczba_poprawek > 0:
		...
		for wiersz in range(rozmiar_planszy):
			for kolumna in range(rozmiar_planszy):
				...
				if plansza_gry[wiersz][kolumna] == 'X':	
					for koord in koordy:
						nowy_wiersz = wiersz+koord[0]
						nowa_kolumna = kolumna+koord[1]
						if (nowy_wiersz > -1) and (nowa_kolumna > -1):
					...
					if miny.count('X') == 8:
						while True:
							wiersz_miny = randint(0, rozmiar_planszy-1)
							kolumna_miny = randint(0, rozmiar_planszy-1)
							if plansza_gry[wiersz_miny][kolumna_miny] == 0:
								plansza_gry[wiersz_miny][kolumna_miny] = 'X'
								break
						liczba_poprawek += 1
...
def przygotuj_plansze(rozmiar_planszy, ile_min):
	...
	plansza = sprawdz_miny(plansza, rozmiar_planszy)
...
###SILNIK CALEJ GRY###	
def zrob_ruch(plansza_gracza, plansza_oryginal, odkryte_pola, rozmiar_planszy, ile_min, grupy_zer, przyciski_planszy):
